...
4. **Manifest & output layout:** unchanged in structure. The manifest file is named `<sanitized target_name>.manifest.json` and videos land in `<output_dir>/<sanitized target_name>/`. For a playlist, `target_name` is the playlist title. Each playlist gets its own manifest (so the same video appearing in two playlists is tracked independently — correct).
